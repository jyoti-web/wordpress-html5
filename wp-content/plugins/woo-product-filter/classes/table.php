...
abstract class tableWpf {
    ...
    static public function getInstance($table = '') {
        ...
        if(!isset($instances[$table])) {
            $class = 'table'. strFirstUp($table). strFirstUp(WPF_CODE);
    ...
    public function store($data, $method = 'INSERT', $where = '') {
        ...
        if(empty($fields)) {
            $this->_addError(__('Nothing to update', WPF_LANG_CODE));
            return false;
        }
        ...
        if(dbWpf::query($query)) {
			...
			$this->_addError(WPF_TEST_MODE ? dbWpf::getError() : __('Database error. Please contact your developer.', WPF_LANG_CODE));
        return false;
    }
                ...
                if(array_key_exists($k, $this->_fields) || $k == $this->_id) {
                    ...
                    if($validate) {
                        if (isset($this->_fields[$k]) && is_object($this->_fields[$k])) {
                            ...
                            if($errors = validatorWpf::_($objForValidation)) {
            ...
            switch($f->type) {
                case 'date':
                    if($d[$key] == '0000-00-00' || empty($d[$key]))
                        ...
                        $d[$key] = date(WPF_DATE_FORMAT, dbWpf::dateToTime($d[$key]));
                    }
                    break;
                case 'int':
                case 'tinyint':
