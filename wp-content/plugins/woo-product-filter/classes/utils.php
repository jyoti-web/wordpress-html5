...
class utilsWpf {
    ...
    static public function copyDirectories($source, $destination) {
        if(is_dir($source)) {
            ...
            while ( FALSE !== ( $readdirectory = $directory->read() ) ) {
                ...
                if (is_dir($PathDir)) {
                    utilsWpf::copyDirectories( $PathDir, $destination . '/' . $readdirectory );
                    continue;
                }
    ...
    static public function xmlNodeAttrsToArr($node) {
        ...
        foreach($node->attributes() as $a => $b) {
            $arr[$a] = utilsWpf::xmlAttrToStr($node, $a);
        }
        return $arr;
    }
    ...
    static public function getPluginPath($name = '') {
        ...
        if(substr($path, 0, 4) != 'http') {
            $home = home_url();
            if(is_ssl() && substr($home, 0, 5) != 'https') $home = 'https'.substr($home, 4);
    ...
    static public function isThisCommercialEdition() {
        ...
        foreach($commercialModules as $m) {
            if(!frameWpf::_()->getModule($m)) 
                return false;
            if(!is_dir(frameWpf::_()->getModule($m)->getModDir())) 
                return false;
        }
        return true;*/
