...
class frameWpf {
        ...
        if($activeModules) {
            foreach($activeModules as $m) {
                ...
                if(is_dir($moduleLocationDir. $code)) {
                    ...
                    if((bool)$m['active']) {
                        importClassWpf($code. strFirstUp(WPF_CODE), $moduleLocationDir. $code. DS. 'mod.php');
	...
	public function connectLang() {
		global $langOK;
		$langOK = load_plugin_textdomain(WPF_LANG_CODE, false, WPF_PLUG_NAME. '/languages/');
	}
    /**
    ...
    public function checkPermissions($code, $action) {
        if($this->havePermissions($code, $action))
            ...
            exit(_e('You have no permissions to view this page', WPF_LANG_CODE));
        }
    }
    /**
    ...
    public function havePermissions($code, $action) {
        ...
        if($mod) {
            ...
            if(!empty($permissions)) {  // Special permissions
                if(isset($permissions[WPF_METHODS])
                    && !empty($permissions[WPF_METHODS])
                ) {
                    ...
                    if(array_key_exists($action, $permissions[WPF_METHODS])) {        // Permission for this method exists
                        $currentUserPosition = frameWpf::_()->getModule('user')->getCurrentUserPosition();
                ...
                if(isset($permissions[WPF_USERLEVELS])
                    && !empty($permissions[WPF_USERLEVELS])
                ) {
                    $currentUserPosition = frameWpf::_()->getModule('user')->getCurrentUserPosition();
	...
	public function isAdminPlugOptsPage() {
		...
		if(is_admin() && strpos($page, frameWpf::_()->getModule('adminmenu')->getMainSlug()) !== false)
			return true;
		return false;
	}
