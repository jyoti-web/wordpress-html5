...
class fieldWpf {
	...
	public function drawHtml($tag, $id) {
		if(method_exists('htmlWpf', $this->html)) {
			$method = $this->html;
			...
			if($this->adapt['html']) {
				fieldAdapterWpf::_($this, $this->adapt['html'], fieldAdapterWpf::HTML);
			}
			$params = $this->processParams($tag, $id);
			if ($params != '')
				return $params;
			if ($this->name == 'default_value') {
				$optionsFromDb = frameWpf::_()->getModule('optionsWpf')->getHelper()->getOptions($id);
				if (!empty($optionsFromDb)) {
					$options = array(0 => __('Select', WPF_LANG_CODE));
					foreach($optionsFromDb as $k => $v)
						$options[$k] = $v;
					$method = 'selectbox';
					$this->htmlParams['optionsWpf'] = $options;
				}
			}
			$htmlContent = htmlWpf::$method($this->name, $this->htmlParams);
			if(!empty($this->errorEl)) {
				if(is_bool($this->errorEl))
					$errorEl = '<div class="toeErrorForField toe_'. htmlWpf::nameToClassId($this->name). '"></div>';
	...
	public function displayValue() {
		...
		switch($this->html) {
			case 'countryList':
				$value = fieldAdapterWpf::displayCountry($this->value);
				break;
			case 'statesList':
				$value = fieldAdapterWpf::displayState($this->value);
				break;
			case 'checkboxlist':
	...
	public function viewField($name, $fieldValue = '') {
		$method = $this->html;
		$options = frameWpf::_()->getModule('optionsWpf')->getHelper()->getOptions($this->id);
		$attrs = '';
		...
		$params = array('optionsWpf'=>$options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked);
		$output = '';
		if(method_exists('htmlWpf', $method)) {
			$output .= htmlWpf::$method($name, $params);
			$output .= htmlWpf::hidden('extra_field['.$this->name.']',array('value'=>$this->id));
		}
		return $output;
	}
	...
	public function processParams($tag, $id){
		...
		if ($this->name == "params") {
			...
			switch ($tag) {
				case 5: 
					$add_option = __('Add Checkbox', WPF_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 9: 
					$add_option = __('Add Item', WPF_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 12:
				...
				case 10:
					$options_tag = '';
					$add_option = __('Add Radio Button', WPF_LANG_CODE);
					$image_tag = ' style="display:none"';
				break;
				case 8:
					$image_tag = '';
			...
			if ($tag > 0 || $id == 0) {
				$output .= '<div class="options options_tag"'.$options_tag.'>';
					...
					$output .= fieldAdapterWpf::_($id,'getExtraFieldOptions',fieldAdapterWpf::STR);
				$output .= '</div>';

				$output .= '<div class="options image_tag"'.$image_tag.'>'.__('Dimensions', WPF_LANG_CODE).':<br />';
					$params->width?$width = $params->width:'';
					$params->height?$height = $params->height:'';
					$output .= __('width', WPF_LANG_CODE).':<br />';
					$output .= htmlWpf::text('params[width]',array('value'=>$width)).'<br />';
					$output .= __('height', WPF_LANG_CODE).':<br />';
					$output .= htmlWpf::text('params[height]',array('value'=>$height)).'<br />';
				$output .= '</div>';
			}
			if($this->adapt['htmlParams']) {
				$output .= fieldAdapterWpf::_($this, $this->adapt['htmlParams'], fieldAdapterWpf::STR);
			} else {
				$output .= '<a href="javascript:void(0);" class="set_properties">'.__('Click to set field "id" and "class"', WPF_LANG_CODE).'</a>';
				$output .= '<div class="attributes" style="display:none;">'.__('Attributes', WPF_LANG_CODE).':<br />';
				$output .= fieldAdapterWpf::_($params,'getFieldAttributes',  fieldAdapterWpf::STR);
				$output .= '</div>';
			}
			return $output;
		}
	...
	public function drawConfig($module, $additionalOptions = array()) {
		if(!frameWpf::_()->getModule($module)) 
			return false; 
		// check for xml file with params structure  
	   if(frameWpf::_()->getModule($module)->isExternal())
		   $config_xml = frameWpf::_()->getModule($module)->getModDir(). 'mod.xml';
	   else
		   $config_xml = WPF_MODULES_DIR.$module.DS.'mod.xml';

	   if (!file_exists($config_xml)) {
		   // if there is no configuration file for this $module
		   return __('There are no configuration options for this module', WPF_LANG_CODE);
	   }
	   $output = '';
		   ...
		   foreach ($params as $param) {
			   ...
			   foreach ($configOptions as $key=>$value){
				  ...
				  // if helper is needed to render the object
					  ...
					  $helper = frameWpf::_()->getModule($hmodule)->getHelper();
					  if ($helper) {
				  ...
				  // filling the parameters to build html element
					 $htmlParams = array('value'=>$fieldValue,'optionsWpf'=>$options);
					 if($method == 'checkbox') {
						 $htmlParams['value'] = 1;
				  ...
				  // output label and html element
					 ...
					 $output .= '</label><br />';
					 $output .= htmlWpf::$method($name,$htmlParams).'<br />';
					 $output .= '</div>';
			   }
			   $i++;
			 $output .= '</div>';
