...
class modInstallerWpf {
        ...
        if(!empty($module) && !empty($path) && is_dir($path)) {
            if(self::isModule($path)) {
                ...
                if($filesMoved) {
                    ...
                    errorsWpf::push(sprintf(__('Move files for %s failed'), $module['code']), errorsWpf::MOD_INSTALL);
                }
            } else
                errorsWpf::push(sprintf(__('%s is not plugin module'), $module['code']), errorsWpf::MOD_INSTALL);
        }
        return false;
    }
    static protected function _runModuleInstall($module, $action = 'install') {
        ...
        if(is_dir($moduleLocationDir. $module['code'])) {
			if(!class_exists($module['code']. strFirstUp(WPF_CODE))) {
				importClassWpf($module['code']. strFirstUp(WPF_CODE), $moduleLocationDir. $module['code']. DS. 'mod.php');
			}
    ...
    static public function moveFiles($code, $path) {
        if(!is_dir(WPF_MODULES_DIR. $code)) {
            if(mkdir(WPF_MODULES_DIR. $code)) {
                ...
                errorsWpf::push(__('Cannot create module directory. Try to set permission to '. WPF_MODULES_DIR. ' directory 755 or 777', WPF_LANG_CODE), errorsWpf::MOD_INSTALL);
        } else
            return true;
        return false;
    }
    ...
    static private function _getModulesFromXml($xmlPath) {
        if($xml = utilsWpf::getXml($xmlPath)) {
            if(isset($xml->modules) && isset($xml->modules->mod)) {
                ...
                if(empty($modules))
                    errorsWpf::push(__('No modules were found in XML file', WPF_LANG_CODE), errorsWpf::MOD_INSTALL);
                else
                    return $modules;
            } else
                errorsWpf::push(__('Invalid XML file', WPF_LANG_CODE), errorsWpf::MOD_INSTALL);
        } else
            errorsWpf::push(__('No XML file were found', WPF_LANG_CODE), errorsWpf::MOD_INSTALL);
        return false;
    }
    /**
        ...
        if($modules = self::_getModulesFromXml($locations['xmlPath'])) {
            foreach($modules as $m) {
                ...
                if(!empty($modDataArr)) {
                    ...
                    if(frameWpf::_()->getTable('modules')->exists($modDataArr['code'], 'code') /*frameWpf::_()->moduleExists($modDataArr['code'])*/) {
                        self::activate($modDataArr);
                    } else {                                           //  if not - install it
                        if(!self::install($modDataArr, $locations['plugDir'])) {
                            errorsWpf::push(sprintf(__('Install %s failed'), $modDataArr['code']), errorsWpf::MOD_INSTALL);
                        }
                    }
                }
            }
        } else
            errorsWpf::push(__('Error Activate module', WPF_LANG_CODE), errorsWpf::MOD_INSTALL);
            ...
            foreach($modules as $m) {
                ...
                if(frameWpf::_()->moduleActive($modDataArr['code'])) { //If module is active - then deacivate it
                    if(frameWpf::_()->getModule('options')->getModel('modules')->put(array(
                        ...
                        errorsWpf::push(__('Error Deactivation module', WPF_LANG_CODE), errorsWpf::MOD_INSTALL);
                    }
                }
            }
        }
            ...
            foreach($modules as $m) {
                ...
                if(!frameWpf::_()->moduleActive($modDataArr['code'])) { //If module is not active - then acivate it
                    if(frameWpf::_()->getModule('options')->getModel('modules')->put(array(
                        ...
                        errorsWpf::push(__('Error Activating module', WPF_LANG_CODE), errorsWpf::MOD_INSTALL);
                    } else {
    ...
    static public function _installTables($module, $action = 'install') {
		$modDir = empty($module['ex_plug_dir']) ? 
            WPF_MODULES_DIR. $module['code']. DS : 
            utilsWpf::getPluginDir($module['ex_plug_dir']). $module['code']. DS; 
