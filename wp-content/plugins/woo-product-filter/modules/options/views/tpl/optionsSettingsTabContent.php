...
<section>
	<form id="wpfSettingsForm" class="wpfInputsWithDescrForm">
						...
						<?php if(isset($optCatData['opts']) && !empty($optCatData['opts'])) { ?>
							<?php foreach($optCatData['opts'] as $optKey => $opt) { ?>
								...
								>
										...
										<?php if(isset($opt['pro']) && !empty($opt['pro'])) { ?>
											<span class="wpfProOptMiniLabel">
												<a href="<?php echo $opt['pro']?>" target="_blank">
													<?php _e('PRO option', WPF_LANG_CODE)?>
												</a>
											</span>
										<?php }?>
									</th>
									...
									<td class="col-w-1perc">
										<?php echo htmlWpf::$htmlType('opt_values['. $optKey. ']', $htmlOpts)?>
									</td>
									<td class="col-w-60perc">
										<div id="wpfFormOptDetails_<?php echo $optKey?>" class="wpfOptDetailsShell">
										<?php switch($optKey) {
		...
		<?php echo htmlWpf::hidden('mod', array('value' => 'options'))?>
		<?php echo htmlWpf::hidden('action', array('value' => 'saveGroup'))?>
	</form>
	<br />
</section>