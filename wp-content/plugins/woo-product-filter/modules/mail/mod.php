...
class mailWpf extends moduleWpf {
		...
		switch($type) {
			...
			case 'wp_mail': default:
				...
				if(empty($ts_mail_errors)) {
					$ts_mail_errors[] = __('Cannot send email - problem with send server', WPF_LANG_CODE);
				}
	...
	public function addOptions($opts) {
		$opts[ $this->getCode() ] = array(
			'label' => __('Mail', WPF_LANG_CODE),
			'opts' => array(
				'mail_function_work' => array('label' => __('Mail function tested and work', WPF_LANG_CODE), 'desc' => ''),
				'notify_email' => array('label' => __('Notify Email', WPF_LANG_CODE), 'desc' => __('Email address used for all email notifications from plugin', WPF_LANG_CODE), 'html' => 'text', 'def' => get_option('admin_email')),
			),
		);
		return $opts;
	}
