    ...
    function importClassWpf($class, $path = '') {
        if(!class_exists($class)) {
            ...
            /*if(class_exists('ReflectionClass')) {   //ReflectionClass supported begining from php5
                ...
                if(strpos($classFile, WPF_DIR) === false) {   //Class is not in our plugin directory
                    ...
                    if(!empty($plugins)) {
                        for($i = 0; $i < count($plugins); $i++) {
                            if(strpos($plugins[$i], WPF_PLUG_NAME) !== false) {   //Let's remove our plugin from list of active plugins
                                unset($plugins[$i]);
                            }
                        }
...
if(!function_exists('toeGetClassNameWpf')) {
    function toeGetClassNameWpf($class) {
        $className = '';
		if(class_exists($class. strFirstUp(WPF_CODE))) 
			$className = $class. strFirstUp(WPF_CODE);
