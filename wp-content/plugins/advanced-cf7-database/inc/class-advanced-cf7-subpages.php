...
class advanced_cf7_subpage{
	...
	public function list_table_page(){
        $ListTable = new ACFDB7_Subpage_List_Table();
 ...
 */
class ACFDB7_Subpage_List_Table extends WP_List_Table
{
    private $form_post_id;
    ...
    private function table_data()
    {
        ...
        foreach ( $results as $result ) {
            ...
            $link  = "<b><a href=admin.php?page=advanced-cf7-listing&fid=%s&ufid=%s>%s</a></b>";
            ...
            foreach ($form_value as $k => $value) {
                ...
                if ( $can_foreach ) {
                    foreach ($value as $k_val => $val):
                        ...
                        $form_values[$ktmp] = sprintf($link, $fid, $result->id, $form_values[$ktmp]);
                    endforeach;
                }else{
            ...
            $form_values['form-date'] = sprintf($link, $fid, $result->id, $result->form_date );
            $data[] = $form_values;
        }
    ...
    public function process_bulk_action(){
        ...
        if( 'delete' === $action ) {
            ...
            foreach ($form_ids as $form_id):
                ...
                $results       = $cfdb->get_results( "SELECT * FROM $table_name WHERE id = $form_id LIMIT 1", OBJECT );
    ...
    protected function bulk_actions( $which = '' ) {
		...
		echo "\n";
        $nonce = wp_create_nonce( 'dnonce' );
        echo "<a href='".$_SERVER['REQUEST_URI']."&excel=true&nonce=".$nonce."' style='float:right; margin:0;' class='button'>";
        echo 'Export Excel';
        echo '</a>';
		echo "\n";
        $nonce = wp_create_nonce( 'dnonce' );
        echo "<a href='".$_SERVER['REQUEST_URI']."&pdf=true&nonce=".$nonce."' style='float:right; margin:0;' class='button'>";
        echo 'Export PDF';
        echo '</a>';
    }
}